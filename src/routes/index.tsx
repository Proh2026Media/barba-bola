            ...
            <section className={`p-8 rounded-[32px] border ${tier.border} text-center relative overflow-hidden transition-all duration-500 group shadow-2xl backdrop-blur-xl bg-white/5 dark:bg-black/40 ${tier.name === 'Exclusive' ? 'border-white/50 shadow-[0_0_60px_rgba(255,255,255,0.2)] ring-1 ring-white/30' : 'border-white/10'}`}>
              ...
              <div className="absolute top-0 right-0 p-3">
                <button 
                  ...
                  className={`h-10 w-10 rounded-2xl flex items-center justify-center border transition-all duration-500 shadow-lg backdrop-blur-lg ${tier.name === 'Exclusive' ? (isDarkMode ? 'bg-hologram-metallic border-transparent' : 'bg-[#050505] border-white/20') : tier.bg + ' ' + tier.border} hover:brightness-110 active:scale-95`}
                >
                  <tier.icon size={20} className={`${tier.name === 'Exclusive' ? (isDarkMode ? 'text-[#050505]' : 'text-white') : tier.colorClass.replace('text-gradient-', 'text-')} drop-shadow-[0_0_8px_currentColor]`} style={tier.name === 'Exclusive' ? { fill: isDarkMode ? '#050505' : 'url(#hologram-gradient)', stroke: 'none' } : {}} />
                </button>
              </div>
              
              <button 
                onClick={() => setShowVipInfo(true)}
                className={`inline-block px-8 py-3 rounded-2xl mb-6 transform transition-all duration-500 hover:brightness-110 active:scale-95 border border-white/20 ${tier.name === 'Exclusive' && !isDarkMode ? 'bg-piano-black shadow-none' : tier.badge + ' shadow-2xl'}`}
              >
                {tier.name === 'Exclusive' && !isDarkMode && <div className="specular-shine" />}
                <h2 className={`text-3xl font-black tracking-tight uppercase drop-shadow-md relative z-10 ${tier.name === 'Exclusive' && !isDarkMode ? 'text-gradient-hologram' : ''}`}>Gabriel Rodrigues</h2>
              </button>
