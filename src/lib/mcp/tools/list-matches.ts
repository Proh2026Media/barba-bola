...
export default defineTool({
  name: "list_matches",
  title: "List matches",
  description: "Return all matches currently tracked in the Arena app (live, finished, and in overtime).",
  inputSchema: {
    league: z.string().optional().describe("Optional league filter, e.g. 'Brasileirão', 'NBA'."),
    status: z.enum(["AO VIVO", "PRORROGAÇÃO", "ENC"]).optional().describe("Optional status filter."),
  },
